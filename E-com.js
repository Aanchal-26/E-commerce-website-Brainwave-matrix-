...
// This will reference the price element on the item page
let itemPrice = document.querySelector(".itemText h3"); 
...
card.forEach(function (curValue) {
  curValue.addEventListener("click", function () {
    ...
    // Get the selected item's image and price
    let imgSrc = curValue.firstElementChild.src;
    let price = curValue.getAttribute("data-price");

    // Update the item page with the correct image and price
    pageImg.src = imgSrc;
    itemPrice.textContent = `$${price}`;  // Dynamically set the price
  });
});

// Handle "Buy Now" button on item page
